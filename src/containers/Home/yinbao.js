    ...
    return (
      <div className={Style.container}>
        <div className = {Style.sheet}  onClick={()=>this.handleChange1()}>
            <div className={Style.push}>银保当日录单</div>
            <div className={Style.arrow}><span className={ Style.bottom} ref='icon1'></span></div>
            <div className={Style.numer} ref='gexian1'><Present code={['025000']}/></div>
        </div> 
        <div className={Style.charts}  ref="myTextInput1"  style={{height:'',overflow:'hidden'}}>
            <div className={Style.more}>
                <span style={{float:'left'}}>数据更新时间({this.props.data && this.props.data["025000"]})</span>    
                <Link style={{float:'right'}} to='yinp'>更多>></Link>        
            </div> 
		  	<PrentPolicyPount indexCode="025000"/>
        </div>
        <div className = {Style.sheet} onClick={()=>this.handleChange2()}>
		 	<div className={Style.push}>银保首年新单保费</div>
            <div className={Style.arrow}><span className={ Style.left} ref='icon2'></span></div>
            <div className={Style.numer}  ref='gexian2'><ListNumber code={['027000']}/></div>
        </div> 
         <div className={Style.charts}  ref="myTextInput2" style={{height:'0',overflow:'hidden'}}>
           <div className={Style.more}>
                <span style={{float:'left'}}>数据更新时间({this.props.data && this.props.data["027000"]})</span>    
                  <Link style={{float:'right'}} to={{pathname:'/test',state:'418'}}>更多>></Link>   
            </div> 
			 <IndexDetails indexCode="027000"></IndexDetails>
        </div>
      </div>
    );
  }
