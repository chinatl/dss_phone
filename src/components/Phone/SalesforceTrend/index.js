    ...
    getOption() {
		...
		if(this.props.data !== undefined){
			...
			if(this.props.data.length == 1){
				var str = '<ul class="'+Style.tableItem+'">'+
						'<li>日期</li>'+
						'<li>当前人力</li>'+
					'</ul>';
				var arr = [];
				var arr1 =  this.props.data[0];
				for(var i=0;i<arr1.time.length;i++){
					arr.push('<ul class="'+Style.txtItem+'">'+
						'<li>'+arr1.time[i]+'</li><li>'+arr1.value[i]+'</li>' +
						 '</ul>')
				}	
				contain.innerHTML = str + arr.join('')
				...
				var arr = [];
				var arr1 =  this.props.data[0];
				var arr2 =  this.props.data[1];
				for(var i=0;i<arr1.time.length;i++){
					arr.push('<ul class="'+Style.txtItem+'">'+
						'<li>'+arr1.time[i]+'</li><li>'+arr1.value[i]+'</li>' +
						 '<li>'+arr2.value[i]+'</li></ul>')
				}	
				contain.innerHTML = str + arr.join('')
			}
		}
