...
//		})
    ...
    changePage(){
  	...
  	render() {
		...
		var classTitle = this.state.data.is_total === '0' ? '' : Style.table;
		if(len && maxNum){
			if(len !== maxNum){
...
//			}
    ...
    return (
      <div style={{padding:'10px',paddingBottom:'0',position:'relative',margin:'0 auto',width:Screenwidth}}
       className={Style.report}>
      <Spin spinning={this.state.loading} size="large" tip="加载中...">
		  { this.props.title === undefined && <h3 style={{textAlign:"center",height:'30px',marginBottom:'15px'}}>{this.state.data.title}</h3>}
		  {this.state.data.term_info && this.state.data.term_info.map((value,index)=>{
			  if(value.type == '4'){
                ...
                <Button 
                	...
                	style={{top:this.props.title === undefined ? '50px' : '0'}}
			...
			{this.state.data.last_update_time && <p style={{margin:'0 0 10px 0',textAlign:'right',color:'#aaa',fontSize:'12px'}}>更新时间({this.state.data.last_update_time})</p>}
	  ...
	  <div className={classTitle} >
		 ...
		 >
			...
			<div style={{marginTop:'20px',color:'#000'}}>
				{this.state.data.remark && <p style={{margin:'10px 0 0 0',color:'#aaa',fontSize:'12px'}}>备注：</p>}
				{this.state.data.remark && this.state.data.remark.split('\n').map(e=>{
					return (<p style={{margin:0,color:'#aaa',fontSize:'12px'}} key={Math.random()}>{e}</p>)
				})}
			</div>
	  	</div>
		</Spin>
